...
class SteamSignIn():
	...
	def GetOwnedGames(self, steam64_id, api_key):
		url = f'https://api.steampowered.com/IPlayerService/GetOwnedGames/v1/?key={api_key}&steamid={steam64_id}'
		response = requests.get(url)
		return response.json()

	def GetUserStatsForGame(self, steam64_id, api_key, app_id):
		url = f'https://api.steampowered.com/ISteamUserStats/GetUserStatsForGame/v2/?key={api_key}&steamid={steam64_id}&appid={app_id}'
		response = requests.get(url)
		return response.json()
